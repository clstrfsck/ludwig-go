...
func TestIsLetter(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, tt.expected, result, "isLetter(%q)", tt.input)
		})
	}
}

// TestKeyIsLower tests the keyIsLower function
func TestKeyIsLower(t *testing.T) {
	// First ensure LowerSet is initialized
	if LowerSet.BitLen() == 0 {
		// Initialize LowerSet with lowercase ASCII letters (97-122)
		for ch := 'a'; ch <= 'z'; ch++ {
			LowerSet.SetBit(&LowerSet, int(ch), 1)
		}
	}
	...
	tests := []struct {
		...
		{"LowercaseA", int('a'), true},
		{"LowercaseB", int('b'), true},
		{"LowercaseM", int('m'), true},
		{"LowercaseZ", int('z'), true},

		// Uppercase letters (ASCII 65-90)
		{"UppercaseA", int('A'), false},
		{"UppercaseB", int('B'), false},
		{"UppercaseM", int('M'), false},
		{"UppercaseZ", int('Z'), false},
		...
		// Special characters
		{"Space", int(' '), false},
		{"Exclamation", int('!'), false},
		{"AtSign", int('@'), false},

		// Edge cases
		{"NegativeKey", -1, false},
		{"NegativeKey100", -100, false},
		{"Zero", 0, false},
		{"MaxSetRange", MaxSetRange, false},
		{"MaxSetRangePlus1", MaxSetRange + 1, false},
		{"MaxSetRangePlus100", MaxSetRange + 100, false},
		{"LargePositive", 10000, false},

		// Boundary values around lowercase range
		{"BeforeLowercaseA", int('a') - 1, false},
		{"AfterLowercaseZ", int('z') + 1, false},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := keyIsLower(tt.input)
			assert.Equal(t, tt.expected, result, "keyIsLower(%d)", tt.input)
		})
	}
}

// TestKeyIsLowerBoundaryConditions tests boundary conditions more thoroughly
func TestKeyIsLowerBoundaryConditions(t *testing.T) {
	// First ensure LowerSet is initialized
	if LowerSet.BitLen() == 0 {
		// Initialize LowerSet with lowercase ASCII letters (97-122)
		for ch := 'a'; ch <= 'z'; ch++ {
	...
	t.Run("AllLowercaseASCII", func(t *testing.T) {
		for ch := 'a'; ch <= 'z'; ch++ {
			result := keyIsLower(int(ch))
			assert.True(t, result, "Expected keyIsLower(%d) [%c] to be true", ch, ch)
		}
	})

	t.Run("AllUppercaseASCII", func(t *testing.T) {
		for ch := 'A'; ch <= 'Z'; ch++ {
			result := keyIsLower(int(ch))
			assert.False(t, result, "Expected keyIsLower(%d) [%c] to be false", ch, ch)
		}
	})
	...
	t.Run("NegativeRange", func(t *testing.T) {
		for key := -10; key < 0; key++ {
			result := keyIsLower(key)
			assert.False(t, result, "Expected keyIsLower(%d) to be false for negative key", key)
		}
	})

	t.Run("BeyondMaxSetRange", func(t *testing.T) {
		for key := MaxSetRange + 1; key <= MaxSetRange+10; key++ {
			result := keyIsLower(key)
			assert.False(t, result, "Expected keyIsLower(%d) to be false for key > MaxSetRange", key)
		}
	})
}
