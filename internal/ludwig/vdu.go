...
func convertColorSchemeToPairs(scheme map[string]string) (*map[string]int, *map[int]ncursesColor) {
	...
	for name, hex := range scheme {
		...
		index, exists := colourIndexes[newColor]
		if !exists {
			// Create a new color pair for this color
			index = pairIndex
			colourIndexes[newColor] = index
			var oldColor ncursesColor
			nc.ColorContent(index, &oldColor.r, &oldColor.g, &oldColor.b)
			oldColorMap[index] = oldColor
...
func vduInitColors() (bool, *map[string]int) {
	...
	colorMap, originalColors := convertColorSchemeToPairs(colorScheme)
...
func VduColorOn(pair int) {
	stdscr.ColorOn(pair)
}

// VduColorOff turns off the specified color pair
func VduColorOff(pair int) {
