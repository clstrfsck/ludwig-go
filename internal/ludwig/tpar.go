...
func TparToInt(strng *TParObject, chpos *int) (int, bool) {
	...
	if ch < '0' || ch > '9' {
		Screen.Message(MsgInvalidInteger)
		return 0, false
	}
	number := 0
	for {
...
func tparSubstitute(tpar *TParObject, cmd Commands, thisTp int) bool {
	if tpar.Con != nil {
		Screen.Message(MsgSpanNamesAreOneLine)
		return false
	}
	...
	if SpanFind(name, &span, &dummy) {
		...
		if startMark.Line == endMark.Line {
			...
			Screen.Message(MsgSpanMustBeOneLine)
			return false
		} else {
		...
		case varTypeLudwig:
			...
			case "COMMAND_INTRODUCER":
				if !ChIsPrintable(rune(CommandIntroducer)) {
					Screen.Message(MsgNonprintableIntroducer)
					return "", true
				} else {
				...
				if LudwigMode != LudwigBatch {
					if cmd == CmdVerify {
						...
						if tran.Len == 0 {
							...
							verifyReply = Screen.Verify(frame, prompt)
						} else {
						...
						prompt := DfltPrompts[CmdAttrib[cmd].TparInfo[thisTp].PromptName]
						tran.Str, tran.Len = Screen.GetLineP(frame, prompt, CmdAttrib[cmd].TpCount, thisTp)
					} else {
						if tran.Con != nil {
							Screen.Message(MsgPromptsAreOneLine)
							return false
						} else {
							prompt := tran.Str.Slice(1, tran.Len)
							tran.Str, tran.Len = Screen.GetLineP(frame, prompt, CmdAttrib[cmd].TpCount, thisTp)
